...
export default function FriendsManagePage() {
  ...
  const handleCopyLink = () => {
    if (!username) return;
    navigator.clipboard.writeText(`huuman.app/u/${username}`);
    setCopied(true);
  ...
  return (
    <div className="flex h-dvh flex-col bg-zinc-950">
      <header className="flex-none border-b border-zinc-800 px-4 py-3 flex items-center gap-3">
        ...
        <h1 className="text-lg font-semibold text-zinc-100">Your friends</h1>
      </header>
          ...
          <section className="space-y-3">
            ...
            <div className="space-y-2">
              {pendingReceived.map((req) => (
                ...
                >
                  <Avatar name={req.user.display_name || req.user.email} />
            ...
            <div className="space-y-2">
              {friends.map((f) => (
                <div
                  ...
                  className="flex items-center gap-3 rounded-xl border border-zinc-800 bg-zinc-900 px-4 py-3"
                >
                  <Avatar name={f.user.display_name || f.user.email} />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-zinc-200 truncate">
                      {f.user.display_name || f.user.email}
                    </p>
                    <p className="text-xs text-zinc-500">
                      connected {formatRelativeTime(f.acceptedAt || f.createdAt)}
                    </p>
                  </div>
                </div>
              ))}
...
function Avatar({ name }: { name: string }) {
  return (
    <div className="flex h-9 w-9 flex-none items-center justify-center rounded-full bg-zinc-700 text-sm font-semibold text-zinc-200">
      {name.charAt(0).toUpperCase()}
    </div>
  );
}

function QrModal({ username, onClose }: { username: string | null; onClose: () => void }) {
  const link = username ? `https://huuman.app/u/${username}` : null;
      ...
      <div className="relative mx-4 w-full max-w-sm rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
        ...
        <div className="flex flex-col items-center gap-4">
          {link && QRCode ? (
            ...
            <p className="text-sm text-zinc-500">Set a username first to get your link</p>
          )}
          {link && (
...
function SearchModal({ onClose, onSent }: { onClose: () => void; onSent: (userId: string) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-zinc-950/95 backdrop-blur-sm">
      <header className="flex-none border-b border-zinc-800 px-4 py-3 flex items-center gap-3">
        ...
        <h1 className="text-lg font-semibold text-zinc-100">Search for friends</h1>
      </header>
