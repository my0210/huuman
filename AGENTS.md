...
1. **Use `.buttonStyle(.glass)` for standalone glass buttons** -- This is Apple's coordinated system for glass material + touch interaction (scale, bounce, shimmer). It handles gesture recognition without conflicts. Never layer `.buttonStyle(.plain)` + `.glassEffect(.interactive())` -- two gesture systems compete and eat taps.
2. **Use `.glassEffect(.regular, in: shape)` for compound views** -- For views containing multiple interactive children (TextField + Button in an HStack), apply static `.glassEffect()` for the material and let children handle their own interactions. Never add `.interactive()` or `.onTapGesture` to parent views containing Buttons or TextFields.
3. **Prefer `.glassEffect()` over custom backgrounds** -- Instead of manual `.background(color, in: shape).overlay(shape.stroke(...))`, use `.glassEffect(in: shape)`. One modifier replaces background + border + material + adaptive shadows.
4. **Use system-adaptive spacing everywhere** -- `.padding(.vertical)` (no value), `.scenePadding(.horizontal)`, `safeAreaBar(edge:)` with default spacing. The system adapts per device, Dynamic Type, and context. Only use explicit values from the 8pt grid when the system default is proven insufficient.
5. **Accept natural height differences between glass elements** -- `.buttonStyle(.glass)` determines its own system height. It won't pixel-match an adjacent `.glassEffect()` capsule, and that's fine -- iMessage's + button is also slightly smaller than the text field. Don't fight system sizing.
...
- **Use .buttonStyle(.glass) for glass buttons, never DIY** -- `.buttonStyle(.glass)` is Apple's coordinated system that handles glass material AND touch interaction (scale, bounce, shimmer) with zero gesture conflicts. Layering `.buttonStyle(.plain)` + `.glassEffect(.interactive())` creates two competing gesture systems that eat each other's taps. For compound views (HStack with TextField + Button), use `.glassEffect(.regular, in: shape)` without `.interactive()` -- the children handle their own interactions. Never add `.onTapGesture` to a parent view containing Buttons or TextFields -- it steals their taps. (2026-03-14)
...
- **Use .buttonBorderShape(.circle) for circular glass buttons** -- `.buttonStyle(.glass)` defaults to capsule shape (wider than tall). To get a circle, add `.buttonBorderShape(.circle)`. Without it, a glass button with a single icon renders as a pill, not a circle. Use `.imageScale(.large)` and a content `.frame()` to control the circle size. (2026-03-14)
- **Use RoundedRectangle for expandable glass containers, not Capsule** -- `Capsule()` sets cornerRadius = min(width,height)/2, which distorts badly when a view expands (e.g. multi-line text field). Use `RoundedRectangle(cornerRadius: token, style: .continuous)` with a fixed design token so corners stay consistent at any height. Define the token (e.g. `composerRadius`) in Layout.swift, not as a magic number. (2026-03-14)
- **Scroll geometry handlers must distinguish user scroll from container resize** -- When `safeAreaBar` content grows (composer expands), `containerSize.height` shrinks, increasing `distanceFromBottom`. A naive handler sets `userScrolledAway = true`, fighting `defaultScrollAnchor(.bottom)`. Track `containerHeight` alongside `isNearBottom` and skip `userScrolledAway` when the container resized. Add a second `onScrollGeometryChange` for `containerSize.height` that explicitly scrolls to bottom when the container shrinks. (2026-03-14)
...
- **Research the reference before coding UI** -- Never guess at dimensions, shapes, or behaviors. Study the reference app (iMessage, ChatGPT, etc.) thoroughly first: screenshot it, measure it, understand the exact shapes, sizes, and interactions. Every shortcut of "it's probably X" costs more time than looking it up. The user will ask "are you even looking closely?" and they'll be right. (2026-03-14)
- **Don't remove standard values to fix proportions** -- When something looks off, understand WHY before changing paddings, spacing, or sizes. Removing standard values (system padding, established spacing) to force a different look makes the UI worse and harder to fix. Add what's missing instead of subtracting what's there. (2026-03-14)
- **Verify visually at every step, compare with reference** -- Don't declare "done" from a quick glance at a compressed screenshot. Compare pixel-by-pixel with the reference. Use `snapshot_ui` for exact measurements. Pull up the software keyboard for realistic testing. Look at button shapes, icon sizes, corner radii, alignment -- all of it. "Close enough" is not enough for premium B2C products. (2026-03-14)
- **Understand system behavior before adding custom logic** -- Trace the full data flow before writing fixes. Custom scroll handlers, geometry watchers, and state flags can fight system behaviors like `defaultScrollAnchor(.bottom)`. Understand what the system already provides, what's blocking it, and fix the interference rather than piling on workarounds. (2026-03-14)
- **Follow the user's process: research, present, implement** -- When the user asks to research first, do it thoroughly. Present findings with specifics before touching code. Don't jump to implementation. Every skipped research step turns into multiple correction cycles that cost far more time. (2026-03-14)
